...
export default function TAddSection() {
  ...
  const [confirmedSections, setConfirmedSections] = useState([]); // Add state for confirmed sections
  ...
  const handleConfirmSelection = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const courseId = localStorage.getItem("selectedCourseId"); // Retrieve the course ID from localStorage

      if (!courseId) {
        setMessage({ type: "error", text: "No course selected. Please select a course first." });
        return;
      }

      const response = await axios.post(
        "http://localhost:4000/api/v1/sections/confirm-sections",
        { courseId, sectionIds: selectedSections },
        {
          headers: {
      ...
      if (response.status === 200) {
        setMessage({ type: "success", text: "Sections successfully associated with the course!" });
        setConfirmedSections(selectedSections); // Update the confirmed sections state
      }
    } catch (error) {
      const errorMessage = error.response?.data?.message || "An error occurred.";
      setMessage({ type: "error", text: errorMessage });
  ...
  return (
    <div className="flex min-h-screen bg-gray-100">
      ...
      <div className="flex-1 ml-64 p-8 overflow-y-auto">
        ...
        {/* Display Confirmed Sections */}
        <div className="mt-8">
          <h2 className="text-2xl font-bold text-gray-800 mb-4">Confirmed Sections</h2>
          {confirmedSections.length > 0 ? (
            <ul className="space-y-4">
              {confirmedSections.map((section) => (
                <li key={section._id} className="p-4 bg-white rounded-lg shadow-md">
            ...
            <p className="text-gray-500">No sections confirmed yet.</p>
          )}
        </div>
      </div>
      <Footer />
